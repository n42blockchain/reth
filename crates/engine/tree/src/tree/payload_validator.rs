...
use crate::tree::{
    ...
    state_root_strategy::{
        prepare_n42_state_root_job, DefaultStateRootStrategy, LazyHashedPostState,
        PayloadStateRootHandle, PayloadStateRootJobContext, StateRootHintStream,
        StateRootJobContext, StateRootStrategy, StateRootUpdateStream,
    },
};
    ...
    Evm: ConfigureEvm<Primitives = N> + 'static,
{
        ...
        Evm: ConfigureEngineEvm<T::ExecutionData, Primitives = N>,
    {
        ...
        // Check the execution-output cache before preparing state-root work. A hit skips EVM and
        // therefore cannot feed a streaming state-root hook.
        ...
        let n42_skip_state_root = reth_evm::n42_skip_state_root();
        let n42_defer_state_root = reth_evm::n42_defer_state_root();
        ...
        let parallel_bal_execution =
            !is_cache_hit && ensure_ok!(self.bal_path_eligible(env.decoded_bal.as_deref()));

        // Prepare the state-root job before execution so it can provide streaming hooks.
        // Cached output has no execution-hook updates. N42 skip/defer modes also avoid spawning
        // sparse-trie workers whose result would be discarded.
        let mut state_root_job = if is_cache_hit || n42_skip_state_root || n42_defer_state_root {
            prepare_n42_state_root_job(
                provider_builder.clone(),
                n42_skip_state_root,
                n42_defer_state_root,
            )
        } else {
        ...
        let (output, senders, receipt_root_rx, built_bal) =
            if let Some((cached_output, cached_senders)) = cache_hit {
                info!(
                    target: "engine::tree::payload_validator",
                    block_number = input.num_hash().number,
                    %block_hash_for_cache,
                    "N42_PAYLOAD_CACHE_HIT: reusing cached execution output"
                );
                // A closed receiver yields `None` below, so post-execution validation derives the
                // receipt root from the cached receipts.
                ...
                if let (Some(metrics), Some(stats)) =
                    (&state_provider_metrics, &state_provider_stats)
                {
        ...
        let n42_skip_root = n42_skip_state_root || n42_defer_state_root;
        ...
        if is_cache_hit && n42_skip_root {
            ...
            let mode = if n42_skip_state_root { "SKIP" } else { "DEFER" };
            info!(
                target: "engine::tree::payload_validator",
            ...
            let hashed_state: LazyHashedPostState =
                reth_tasks::LazyHandle::ready(Arc::new(HashedPostState::default()));
        ...
        let n42_root_skipped =
            n42_skip_state_root || (n42_defer_state_root && block.header().state_root().is_zero());
        if n42_root_skipped {
            let mode = if n42_skip_state_root { "SKIP" } else { "DEFER" };
            info!(
                target: "engine::tree::payload_validator",
                block_number = block.header().number(),
                mode,
                "N42_STATE_ROOT_SKIPPED: using header state root"
            );
        }
