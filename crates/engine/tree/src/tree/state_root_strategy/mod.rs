...
/// Prepares the non-streaming state-root job required by N42's execution-output cache modes.
///
/// A cache hit has no per-transaction execution hook updates, so it must compute the root from the
/// complete cached output. Skip mode returns the header root directly. Defer mode does the same for
/// the zero-root sentinel used by deferred blocks, but still computes a synchronous root for normal
/// headers.
pub(crate) fn prepare_n42_state_root_job<N, P>(
    provider_builder: StateProviderBuilder<N, P>,
    skip_state_root: bool,
    defer_state_root: bool,
) -> PreparedStateRootJob<N>
where
    N: NodePrimitives,
    P: BlockReader + StateProviderFactory + StateReader + Clone + Send + Sync + 'static,
{
    let job: Box<dyn StateRootJob<N>> = if skip_state_root {
        Box::new(SkippedStateRootJob {})
    } else if defer_state_root {
        Box::new(N42DeferredStateRootJob { provider_builder })
    } else {
        Box::new(SynchronousStateRootJob { provider_builder })
    };
    PreparedStateRootJob::new(job, None)
}
    ...
    P: BlockReader + StateProviderFactory + StateReader + Clone + Send + Sync + 'static,
{
    ...
    fn finish(
        ...
        finish_synchronous_state_root(&self.provider_builder, hashed_state)
    }
}

#[derive(Debug)]
struct N42DeferredStateRootJob<N: NodePrimitives, P> {
    provider_builder: StateProviderBuilder<N, P>,
}

impl<N, P> StateRootJob<N> for N42DeferredStateRootJob<N, P>
where
    N: NodePrimitives,
    P: BlockReader + StateProviderFactory + StateReader + Clone + Send + Sync + 'static,
{
...
fn finish_synchronous_state_root<N, P>(
    provider_builder: &StateProviderBuilder<N, P>,
    hashed_state: &LazyHashedPostState,
) -> ProviderResult<StateRootJobOutcome>
where
...
{
    let provider = provider_builder.clone().build()?;
