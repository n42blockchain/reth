...
use super::{
    ...
    CacheAccount, StateBuilder, StateReadObserver, TransitionAccount, TransitionState,
};
...
pub struct State<DB> {
    ...
    /// Sees every read this state answers and every account it commits, in
    /// order. See [`StateReadObserver`]. `None` costs nothing.
    #[debug(skip)]
    pub read_observer: Option<Box<dyn StateReadObserver>>,
}
...
impl<DB: Database> State<DB> {
    ...
    /// Sets the observer of this state's reads and commits.
    #[inline]
    pub fn set_read_observer(&mut self, observer: Option<Box<dyn StateReadObserver>>) {
    ...
    fn storage(&mut self, address: Address, index: StorageKey) -> Result<StorageValue, DB::Error> {
        ...
        let value = account
            ...
            .transpose()?
            .unwrap_or_default();
        if let Some(observer) = self.read_observer.as_mut() {
            observer.storage(address, index, value);
        }
        Ok(value)
    }
}
...
impl<DB: Database> Database for State<DB> {
    ...
    fn basic(&mut self, address: Address) -> Result<Option<AccountInfo>, Self::Error> {
        ...
        if let Some(observer) = self.read_observer.as_mut() {
            observer.basic(address, basic.as_ref());
        }
        Ok(basic)
    }
    ...
    fn commit(&mut self, changes: AddressMap<Account>) {
        ...
        if let Some(observer) = self.read_observer.as_mut() {
            for (address, account) in &changes {
                observer.committed(*address, account);
            }
        }
